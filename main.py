...
from config import GEMINI_API_KEY, KAIROS_VAULT_DIR, VOICE_NAME
...
def make_tool_wrapper(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            loop.create_task(broadcast({"type": "log", "sender": "SKILL-EXEC", "message": msg}))
        except RuntimeError:
            pass
        ...
        try:
            loop = asyncio.get_running_loop()
            loop.create_task(broadcast({"type": "log", "sender": "SKILL-EXEC", "message": res_msg}))
...
async def process_recorded_audio():
    ...
    try:
        ...
        if not transcription:
            await broadcast(
                {"type": "log", "sender": "SYSTEM", "message": "No se reconoció voz o el texto está vacío."}
            )
...
async def process_user_query(text: str):
    ...
    if not chat_session:
        err_msg = (
            "Error: El cerebro (API de Gemini) no está configurado. Registra tu GEMINI_API_KEY en el archivo .env."
        )
        await broadcast({"type": "log", "sender": "ERROR", "message": err_msg})
        await broadcast({"type": "status", "state": "idle"})
        return
            ...
            try:
                ...
                if any(x in err_str for x in ["503", "UNAVAILABLE", "ResourceExhausted", "429", "rate limit"]):
                    if attempt < max_retries - 1:
                        print(
                            f"[GEMINI] Servidor ocupado/saturado. Reintentando en {retry_delay}s (intento {attempt + 1}/{max_retries})..."
                        )
                        await broadcast(
                            {
                                "type": "status",
